...
export default{
	...
	methods:{
		login(){
			...
			this.axios.post('login',this.logindata).then(response=>{
				if(this.lStorage('token',response)){
					this.AItip('登录成功')
					this.$store.state.token = response
					this.$router.replace('/')
				}else{
		...
		getLoginCode(){
			...
			this.axios.post('vercode',{user:this.logindata.user}).then(response=>{
				this.AItip('验证码发送成功')
			})
		},
	}
