		...
		<q-card style="min-width:1000px">
			...
			<q-item v-for="item,index in record" v-if="index<len">
				<div class="row full-width items-center">
					<div class="col-1"><div v-if="!link">{{item.aid}}</div></div>
					<div class="col-8 row items-center">
						<div class="col-10">
							<div>{{item.title}}</div>
							<div v-if="intro" class="text-caption text-grey-6">{{item.intro}}</div>
							<div v-if="link" class="text-caption text-grey-6">{{item.url}}</div>
						</div>
						<div class="col-2">
					...
					<div class="col-3 text-right row justify-end">
						<q-btn icon="code" dense size="12px" class="q-ml-sm">
							<q-popup-edit v-model="test" class="inin">
				              	...
				              	<q-input v-if="link" v-model="item.url" class="inin" color="red-10" dense autofocus label="链接"/>
				            </q-popup-edit>
						</q-btn>
						<upbtn icon="image_search" dense size="12px" class="q-ml-sm" :iid="'t'+index" @change="function(imgs){item.img=imgs[0]}"/>
						<q-btn v-if="!link" icon="link" dense size="12px" class="q-ml-sm" @click="goSelect(index)"/>
			...
			<q-card-actions align="right">
		        ...
		        <q-btn class="q-px-sm q-ml-sm" flat dense icon="playlist_add" label="批量新增" v-if="!link" @click="goSelect(-1,len-record.length)" :disable="disable"/>
...
data:()=>({
	...
	mid:0,cid:0,index:0,faceimg:0,len:0,intro:0,link:0,
	record:[],
	old:{}
}),
mounted(){
...
methods:{
	showd(mid,data,cid,len=6,faceimg=0,intro=0,link=0){
		this.mid = mid
		this.cid = cid
		this.len = len
	...
	saveContent(){
		...
		this.$emit('changeContent',this.mid,res)
		this.show = false
	},
	sortUp(i){
		if(i==0){return;}
