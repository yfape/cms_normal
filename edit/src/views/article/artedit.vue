...
.uploadimg{max-height:150px;overflow-y:hidden;position:relative;transition:filter 0.5s;cursor:pointer;width:100%;}
.uploadimg:hover{filter: drop-shadow(0px 0px 3px #635C5C);}
.fg{position:relative;width:100%;height:100%;z-index:10;top:-100%;cursor:pointer;}
.fg:hover{background:rgba(88,86,86,0.5)}
...
<div class="row bg-grey-4 full-height" style="overflow:hidden;" v-if="show">
    ...
    <q-tab-panels v-model="pagecontrol.editormode" class="q-px-md bg-grey-2 full-height"  style="width:325px;" keep-alive>
      ...
      <q-tab-panel name="发布" class="column">
        <div class="col-11">
          ...
          <!--栏目选择-->
          <q-input standout="bg-grey-6 text-white" dense class="q-mb-md" color="red-10" v-model="$store.state.system.cols['c'+data.col].col">
            <template v-slot:prepend>
              <span style="font-size:15px">栏目｜</span>
            </template>
            <q-popup-proxy style="width:250px">
                <q-tree dense style="width:250px"
                  :nodes="$store.state.system.webcols"
                  :selected.sync="test"
                  default-expand-all
                  node-key="cid"
                  label-key="col"
                >
                  <template v-slot:default-header="prop" disable>
                    <q-item dense :disable="!prop.node.sub" class="full-width" clickable @click="data.col=prop.node.sub?prop.node.cid:data.col">
                      <q-item-section>
                        {{ prop.node.col }}
                      </q-item-section>
                    </q-item>
        ...
        <div class="col-1 row items-end">
          <q-btn dense class="full-width" color="grey-6" :label="data.aid==0?'新增至草稿箱':'保存至草稿箱'" @click="saveArticle(0)"/>
  ...
  <div class="rightmain row justify-center items-center text-center">
    <froala id="feditor" :tag="'textarea'" :config="webeditor.config" v-model="data.content"></froala>
  </div>
...
export default{
    ...
    return {
      ...
      webeditor:{
        config:{
          ...
          toolbarButtons: {
            'moreText': {
              'buttons': ['clearFormatting','fontFamily','fontSize','bold','italic','underline', 'strikeThrough', 'subscript', 'superscript', 'textColor', 'backgroundColor', 'inlineClass', 'inlineStyle']
            },
            'moreParagraph': {
              'buttons': ['alignLeft', 'alignCenter','indent','alignJustify','alignRight','formatOLSimple','formatOL', 'formatUL', 'paragraphFormat', 'paragraphStyle', 'lineHeight', 'outdent', 'indent', 'quote']
            },
            'moreRich': {
              'buttons': ['insertLink', 'specialCharacters', 'emoticons', 'insertTable', 'insertHR']
            },
            'moreMisc': {
          ...
          fontFamily:{
            'YaHei,sans-serif':'微软雅黑',
            'SimSun,sans-serif':'宋体',
            'FangSong,sans-serif':'仿宋',
            'KaiTi,sans-serif':'楷体',
            'SimHei,sans-serif':'黑体',
          },
          fontSize: ['10', '12', '14', '16', '18', '24', '30', '36','44'],
          quickInsertEnabled:false,
          imageResizeWithPercent: true,
          fileUploadURL:'http://api.scdjw.net/uploadOne',
          fileUploadParams: {
  ...
  methods:{
    setEditorHeight(){
      ...
      this.webeditor.config.heightMax = height
    },
    getArticle(){
      this.$q.loading.show({message: '文章数据加载中'})
      this.axios.post('article',{aid:this.aid}).then(response=>{
        ...
        this.data.createdate = this.timeDate(this.data.createdate)
    ...
    insertEditor(img){
      this.feditor.html.insert('<p style="max-width:100%;text-align:center"><img style="max-width:100%;" src="'+img+'"/></p>')
    },
    inIframe(code){
      this.feditor.html.insert(code)
    },
    submitBaiduCheck(){
    ...
    saveArticle(status){
      ...
      this.axios.post('saveArticle',{article:subjson}).then(response=>{
        this.$q.loading.hide()
