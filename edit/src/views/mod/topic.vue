...
<template><div v-if="show">
	<div>
		...
		<div class="leftp q-px-md full-height bg-white shadow-2" style="position:fixed">
			...
			<div class="q-mt-sm">
				<q-input filled label="链接" :value="pid==-1?'未保存，暂无链接':($store.state.domain.web+'/topic/'+pid)" readonly dense color="red-10"/>
			</div>
			<div class="q-mt-sm">
			...
			<q-select dense filled class="q-mt-sm" color="red-10" v-model="topic.header" :options="headers" label="头部导航" emit-value map-options option-value="value" option-label="text"/>
			<q-select dense filled class="q-mt-sm" color="red-10" v-model="topic.footer" :options="footers" label="底部导航" emit-value map-options option-value="value" option-label="text"/>
		    <q-item tag="label" dense class="q-mt-sm q-mb-sm bg-grey-2 q-pa-sm">
		    ...
		    <q-btn label="保存" v-if="topic.pid==-1" color="grey-7" dense class="full-width" icon="save" @click="addtopic"/>
		    <q-btn label="保存" v-if="topic.pid!=-1" color="grey-7" dense class="full-width" icon="save" @click="savetopic"/>
		    <q-btn label="删除专题" v-if="topic.pid!=-1" color="red-10" dense class="full-width q-mt-sm" icon="delete">
		...
		<div class="rightp q-pa-md">
			<div class="bg-white" id="code">
				<header1 v-if="topic.header==1" :nav="nav"/>
				<header2 v-if="topic.header==2" :nav="nav"/>
				<header4 v-if="topic.header==3" :nav="nav"/>
				...
				:style="{
					...
					>
						...
						:style="{
							...
							<!-- col内组件遍历 -->
							<div draggable="true" v-for="item2,index2 in item1.content" 
							:class="[
...
import header1 from '../page/lib/components/header/header1'
import header2 from '../page/lib/components/header/header2'
import header4 from '../page/lib/components/header/header4'
import footer1 from '../page/lib/components/footer/footer1'
...
export default{
name:'topic',
components:{diaimgtext2,diaimgtext3,upbtn,header1,header2,header4,footer1,diarow,diacol,diaimg,diacarousel,diatextblock},
props:['pid'],
data:()=>({
	show:false,test:'',diamain:'12',nav:[],
	storindex:{index:0,index1:0,index2:0},
	carousel:{},
	topic:{
		pid:-1,title:'',img:'',backimg:'',isheader:1,isfooter:1,backcolor:'#fff',valid:0,
		content:[]
	},
...
methods:{
	initd(){
		this.topic = Object.assign({},{},{
			pid:-1,title:'',img:'',backimg:'',isheader:1,isfooter:1,backcolor:'#fff',valid:0,content:[]
		})
		this.show = true
	},
	getNav(){
	...
	savetopic(){
		...
		this.besmall()
		this.$q.loading.show()
		this.axios.post('savePage',this.topic).then(res=>{
			this.$q.loading.hide()
	...
	addtopic(){
		...
		this.besmall()
		this.$q.loading.show()
		this.axios.post('addPage',this.topic).then(res=>{
			this.$q.loading.hide()
			this.$router.replace('/pages')
		}).catch(err=>{
			this.$q.loading.hide()
	...
	deletetopic(){
		this.$q.loading.show()
		this.axios.post('deletePage',{pid:this.topic.pid}).then(res=>{
			this.$q.loading.hide()
			this.$router.replace('/pages')
		}).catch(err=>{
			this.$q.loading.hide()
