...
class AipImageCensor extends AipBase{
    ...
    public function textCensorUserDefined($textarr){
        $end = [];
        foreach ($textarr as $key => $value) {
            $data = array('text'=>$value);
            $res = $this->request($this->textCensorUserDefinedUrl, $data);

            if(isset($res['error_code'])||!isset($res['conclusion'])||!isset($res['data'])||$res['conclusion']=='合规'){
                continue;
            }
            foreach ($res['data'] as $k => $v) {
                foreach ($v['hits'] as $k1 => $v1) {
                    array_push($end,[
                        'conclusion'=>$v['conclusion'],
                        'msg'=>$v['msg'],
                        'words'=>array_unique($v1['words'])
                    ]);
                }
            }
        }
        ...
        for($i=0;$i<count($end);$i++){
            ...
            for($j=$i+1;$j<count($end);$j++){

                if(!isset($end[$j])){
                    continue;
                }
                if($end[$i]['conclusion']==$end[$j]['conclusion']&&$end[$i]['msg']==$end[$j]['msg']){
                    $end[$i]['words'] = array_unique(array_merge($end[$i]['words'],$end[$j]['words']));
                    unset($end[$j]);
                }
            }
            array_push($endarr,$end[$i]);
        }

        return count($endarr)===0?true:$endarr;
    }
